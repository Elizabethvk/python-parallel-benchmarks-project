...
# Define a computationally intensive function for testing
def matrix_calculation(_unused: int) -> float:
    try:
        ...
        print("Matrix inversion failed. Returning 0 ms.")
        return 0.0
...
def main(samples: int = 5, ext_seed: Optional[int] = None) -> None:
    print("Method: Parallel with multiprocessing")
    print(f"Performing {samples} runs using {min(cpu_count(), 8)} processes")
    ...
    with Pool(processes=min(cpu_count(), 8)) as pool:  # Use up to 8 processes or available CPUs
        times = pool.map(matrix_calculation, range(samples))
